...
namespace AnalyzeExample
{
    ...
    {      
        ...
        public static List<FinalTag> firstJoiner(List<FinalTag> tagits, int index, out int minus, string firstWord1, string fistWord2, string lastWord1, string lastWord2, string timeIteration)
        {
        ...
        /// טיפול במילים בודדות
        /// </summary>
        /// <param name="finalTags">רשימת התגיות הסופית</param>
        ...
        public static List<FinalTag> secondJoinerNotAlone(List<FinalTag> finalTags, int index, out int minus, string firstWord1, string firstWord2, string lastWord1, string lastWord2)
        {
            //בדיקת רציפות ראשונה ובסיסית, לא כוללנית. שימי לב בעקר לליכוד מילים כפולות ורב מילות הקישור אך לא כולן כי הן כבר בתוככי התגית

            if (index < 0 || index + 1 > finalTags.Count)//אם התקבלו ערכים לא תקינים, אינדקס לא קיים
            {
                minus = 0;
                return finalTags;
            }
            ...
            if (index != 0 && index != finalTags.Count - 1)//אם התגית לא ראשונה/אחרונה 
            {

                prev = finalTags[index - 1].InformTags;//תגית קודמת 
                next = finalTags[index + 1].InformTags;//תגית הבאה
                current = finalTags[index].InformTags;//תגית נוכחית
                ...
                //המילה אכן בודדה
                {
                    if (next.Count > 1)//בודדה ואחריה רצף
                    {
                        ...
                        {
                            joinTagBackward(finalTags, index, "timeIteration");//התלכדות עם התגית הבאה
                            minus = 1;
                            return finalTags;
                        }
                        else//המילה הנוכחית לא חיבת לפתח או שהבאה חיבת לפתח
                        {
                            ...
                            {
                                joinTagForward(finalTags, index);
                                minus = 0;
                                return finalTags;
                            ...
                            else//ברירת מחדל, אם המילה לא מקבוצת פותחות, סוגרות, בהתאם לבדיקות. - התלכדות לתגית הארוכה יותר
                            {
                                ...
                                {
                                    joinTagBackward(finalTags, index, "timeIteration");
                                    minus = 1;
                                    return finalTags;
                                    //התלכדות עם התגית הקודמת
                                }
                    ...
                    else//יש אחריה בודדות
                    {
                        int i = 0;
                        int count = 1;
                        for (i = index + 1; i < finalTags.Count - 2 && (finalTags[i].InformTags.Count < 2); i++)//ספירת מילים בודדות
                        {
                            count++;
                            //להסתכל אם נכנס לתנאי הפור...
                            if (mustStart(finalTags[i].InformTags[0].Word, finalTags[i].InformTags[0].Ana) || finalTags[i].InformTags[0].Word == firstWord1 || finalTags[i].InformTags[0].Word == firstWord2)
...

                            {
                                ...
                                if (count == 2)
                                {
                                    finalTags = joinTagBackward(finalTags, index, "dfs");
                                    minus = 1;
                                    return finalTags;
                                }
                                ...
                                else//סגירת כל מי שהיו עד עכשיו לתגית,
                                {
                                    ...
                                    {
                                        finalTags = joinTagForward(finalTags, index);
                                    }
                                    minus = 0;
                                    return finalTags;
                                }
                            ...
                            if (count == 2)
                            {
                                ...
                                {
                                    finalTags = joinTagBackward(finalTags, index, "dfsdf");
                                    minus = 1;
                                    return finalTags;
                                }
                                else
                            ...
                            if (getListEnd(finalTags[i].InformTags[0].Word) || finalTags[i].InformTags[0].Word == lastWord1 || finalTags[i].InformTags[0].Word == lastWord2)//יש מילה שחיבת לסגר, התלכדות כולל היא
                            {
                        ...
                        finalTags = joinTagBackward(finalTags, index, "" + index);//אם יש מילה אחת אחריו, התלכדות. הרי לא נכנס ללולאה בכלל
                        minus = 1;
                        return finalTags;
                    }
            ...
            {
                if (index == 0 && finalTags.Count > 1)//כאשר המילה הבודדה מופיעה בתגית הראשונה
                {
                    finalTags = joinTagForward(finalTags, index);//מילה ראשונה לא בודדה לעולם

                }
                if (index == finalTags.Count - 1 && finalTags.Count > 1)//כשהמילה הבודדה הנה התגית המסימת את השיר
                {
                    finalTags = joinTagBackward(finalTags, index, "" + index);//מילה אחרונה לא בודדה לעולם
                }
            }
            minus = 0;
            return finalTags;
        ...
        /// הפרדת 'והיא' מתגיות 'ו
        /// </summary>
        /// <param name="finalTags">רשימת תגיות סופית</param>
        ...
        public static List<FinalTag> thirdSplit(List<FinalTag> finalTags, int index, out int minus, string firstWord1, string firstWord2, string lastWord1, string lastWord2)
        {
            //בדיקת רציפות ראשונה ובסיסית, לא כוללנית. שימי לב בעקר לליכוד מילים כפולות ורב מילות הקישור אך לא כולן כי הן כבר בתוככי התגית
            ...
            List<Tagit> current = finalTags[index].InformTags;//תגית נוכחית
            for (int i = 0; i < current.Count; i++)
            {
                if (getListSplit(current[i].Word) == true || ((long)Analizing.vav & (long)current[i].Ana) > 0)
                {
                    finalTags = splitTagStart(finalTags, index, i);

                    if (i == 1 && index != 0)//כאשר התגית התפצלה החל מאינדקס אחד ונותרה מילה בודדה
                    {
                        finalTags = joinTagBackward(finalTags, i, "");//יש לאחדה עם התגית הקודמת
                        minus = 1;//לא חובה
                    }
                    else
                        minus = 0;
        ...
        /// <param name="finalTags">רשימת תגיות סופית</param>
        /// <param name="index">האינדקס</param>
        /// <returns>רשימת התגיות המעודכנת לפי צירוף לתגית הבאה</returns>
        ///
        public static List<FinalTag> joinTagForward(List<FinalTag> finalTags, int index)
        {

            finalTags[index].FullTag += " " + finalTags[index + 1].FullTag;//שרשור התגית הבאה לתגית הזו
            foreach (Tagit i in finalTags[index + 1].InformTags)//העתקת נתוחי כל מילות התגית הבאה לרשימת נתוחי התגית הזו
            {
                finalTags[index].InformTags.Add(new Tagit(i.Ana, i.Word));
            }
            finalTags.RemoveAt(index + 1);//הסרת התגית המיותרת
        ...
        public static List<FinalTag> joinTagAround(List<FinalTag> finalTags, int index)
        {
            finalTags[index].FullTag = finalTags[index - 1].FullTag + " " + finalTags[index].FullTag + " " + finalTags[index + 1].FullTag;//השמת שרשור התגית הקודמת, הנוכחית והבאה לתגית הנוכחית
            List<Tagit> cloneList = finalTags[index].InformTags.GetRange(0, finalTags[index].InformTags.Count);//עותק של ניתוחי התגית הנוכחית
            finalTags[index].InformTags = new List<Tagit>();//איפוס רשימת הניתוחים הנוכחית
            foreach (Tagit i in finalTags[index - 1].InformTags)//הוספת נתוחי התגית הקודמת לרשימת הנתוחים הנוכחית הריקה
            {
                finalTags[index].InformTags.Add(new Tagit(i.Ana, i.Word));
            }
            ...
            foreach (Tagit i in finalTags[index + 1].InformTags)//הוספת נתוחי התגית הבאה לרשימה
            {
                finalTags[index].InformTags.Add(new Tagit(i.Ana, i.Word));
            }
            finalTags.RemoveAt(index + 1);//הסרת התגית הבאה
            finalTags.RemoveAt(index - 1);//הסרת התגית הקודמת
            return finalTags;
        ...
        /// </summary>
        /// <param name="finalTags"></param>
        /// <param name="index"></param>
        /// <returns>רשימת התגיות הסופיות לאחר איחוד עם קודמתה </returns>
        public static List<FinalTag> joinTagBackward(List<FinalTag> finalTags, int index, string timeIteration)

        {

            finalTags[index].FullTag = finalTags[index - 1].FullTag + " " + finalTags[index].FullTag;//שרשור תגית נוכחית לתגית הקודמת
            List<Tagit> cloneList = finalTags[index].InformTags.GetRange(0, finalTags[index].InformTags.Count);//יצירת עותק מנתוחי התגית הנוכחית
            finalTags[index].InformTags = new List<Tagit>();//איפוס רשימת הנתוחים של התגית הנוכחית
            foreach (Tagit i in finalTags[index - 1].InformTags)//השמת רשימת נתוחי התגית הקודמת לרשימת התגיות הנוכחית הריקה
            {
                finalTags[index].InformTags.Add(new Tagit(i.Ana, i.Word));
            }
        ...
        public static List<FinalTag> splitTagStart(List<FinalTag> finalTags, int index, int j)
        {
            int wordsTagCount = finalTags[index].InformTags.Count;//מספר המילים בתגית
            if (j > 0 && j < wordsTagCount - 1)//המילה אינה הראשונה/האחרונה בתגית
            {
                {
                    int i;
                    string newNext = finalTags[index].FullTag;//טקסט כלל התגית
                    ...
                    finalTags[index].FullTag = newFirst;//השמת התגית העדכנית במיקום הנכון

                    List<Tagit> tagits = finalTags[index].InformTags.GetRange(j, finalTags[index].InformTags.Count - j);// שרשור הניתוח של שאר המילים-לתגית הבאה
                    finalTags.Insert(index + 1, new FinalTag(newSecond, tagits));//השמת ערכי התגית הבאה וניתוחה
                    finalTags[index].InformTags.RemoveRange(j, finalTags[index].InformTags.Count - j);//השמת הניתוח ללא הניתוחים המיותרים שהועברו לתגית הבאה
                }
            ...
            {
                if (j == wordsTagCount - 1 && index != finalTags.Count - 1)//המילה היא אחרונה בתגית ויש תגית אחר כך
                {
                    finalTags = joinTagForward(finalTags, j);//הוספת המילה קדימה לתגית הבאה
                }

                if ((index == finalTags.Count - 1) && (j == wordsTagCount))// אם אחרונה ללא תגית אחריה
                {
                ...
                if (j == 0 && index != 0)
                {
                    ...
                    newSecond = newSecond.Substring(0, newSecond.Length - 1);//הסרת הרווח המיותר האחרון
                    finalTags[index].FullTag = newFirst;//השמת התגית הבודדה במיקום הנכון
        ...
        /// </summary>
        /// <param name="finalTags">רשימת התגיות</param>
        /// <param name="index">אינדקס</param>
        /// <param name="minus">מספר השנויים</param>
        /// <param name="firstWord1">מילה פותחת פזמון ראשי</param>
        ...
        public static List<FinalTag> thirdSplit8Letters(List<FinalTag> finalTags, int index, out int minus, string firstWord1, string firstWord2, string lastWord1, string lastWord2)
        {
                    ...
                    if ((l == 9) || (l == 10) || (l == 11) || (l == 12) || (l == 13) || (l == 14) || (l == 15) || (l == 16))
                    {
                        ...
                        for (i = c; i > 1; i--)//נסיון לפצל את התגית מהחתך עד המילה השניה בתגית בסדר יורד
                        {
                            ...
                            if (((i != current.Count - 1
                                ...
                                // && ((long)current[i].Ana & (long)Analizing.vav) == 0 
                                ...
                                // && ((long)current[i + 1].Ana & (long)Analizing.shin) == 0 
                                // && ((long)current[i + 1].Ana & (long)Analizing.he) == 0 
                                // && ((long)current[i + 1].Ana & (long)Analizing.kaf) == 0 
                                //&& ((long)current[i + 1].Ana & (long)Analizing.lamed) == 0 
                                // && ((long)current[i + 1].Ana & (long)Analizing.vavCon) == 0
...

                            {
                                ...
                                finalTags = splitTagStart(finalTags, index, i);//פיצול התגית
                                minus = 0;
                                //Console.WriteLine("---------------");
                                return finalTags;
                            }
                        ...
                        //    )// אם המילה לא פותחת פזמון/חיבת אמצע/כפולה פעמיים/צווי/נסמך ושהמילה הבאה אינה חיבת להיות באמצע, משהוכלב, לא חיבת לסגר ולא בפזמונים סוגרים 
                        {
                            finalTags = splitTagStart(finalTags, index, i + 1);
                            finalTags = joinTagBackward(finalTags, index, "fsdf");
                        ...
                        for (i = c; i < c + 2; i++)//לולא שמנסה לפצל את התגית ב2 המקומות שאחרי נקודת הפיצול, הנחה ל10 תווים
                        {
                            ...
                            {
                                finalTags = splitTagStart(finalTags, index, i - 1);//הפיצול
                                minus = 0;// Console.WriteLine("-----------------------------------------------------------------------------------");
                                return finalTags;
                            }
                        }
                        finalTags = splitTagStart(finalTags, index, c);//אין ברירה-חותכים בבשר החי, למרות שלכאורה אסור לחתך
                        minus = 0;
                        // Console.WriteLine("=====================================================");
                        return finalTags;
                    ...
                    if ((l == 24) || (l == 23) || (l == 22) || (l == 21) || (l == 20) || (l == 19) || (l == 18) || (l == 17))//מספר המילים בתגית
                    {
                        ...
                        for (i = c; i > 1; i--)//נסיון לפצל את התגית מהחתך עד המילה השניה בתגית בסדר יורד
                        {
                            ...
                            if (
                                ...
                                && getListMiddleMust(current[i + 1].Word) != true
                                // && ((long)current[i].Ana & (long)Analizing.mem) == 0 
                                // && ((long)current[i + 1].Ana & (long)Analizing.shin) == 0 && ((long)current[i + 1].Ana & (long)Analizing.he) == 0 
                                // && ((long)current[i + 1].Ana & (long)Analizing.kaf) == 0 && ((long)current[i + 1].Ana & (long)Analizing.lamed) == 0 
                                // && ((long)current[i + 1].Ana & (long)Analizing.vavCon) == 0
                                )// אם המילה לא פותחת פזמון/חיבת אמצע/כפולה פעמיים/צווי/נסמך ושהמילה הבאה אינה חיבת להיות באמצע, משהוכלב, לא חיבת לסגר ולא בפזמונים סוגרים
                            {
                                finalTags = splitTagStart(finalTags, index, i);//פיצול התגית
                        ...
                        //    && ((long)current[i].Ana & (long)Analizing.doo) == 0
                        //    && current[i].Word != firstWord1 && current[i].Word != firstWord2
                        //    && getListStart(current[i].Word) != true && getListMiddleMust(current[i].Word) != true
                        //    && ((long)current[i].Ana & (long)Analizing.vav) == 0 &&
                        //       current[i + 1].Word != lastWord1 && current[i + 1].Word != lastWord2
                        //       && getListEnd(current[i + 1].Word) != true
                        //       && getListMiddleMust(current[i + 1].Word) != true
                        ...
                        {
                            finalTags = splitTagStart(finalTags, index, i + 1);
                            finalTags = joinTagBackward(finalTags, index, "kjlj");
                            //    Console.WriteLine("vvvvvvvvvvvvvv");
                            minus = 1;
                            return finalTags;
                        }
     ...
     // && ((long)current[i - 1].Ana & (long)Analizing.vav) == 0
        ...
        static void Main(string[] args)
        {
            ...
            Console.Write("finished! thank you, my crator! 😊 ");
